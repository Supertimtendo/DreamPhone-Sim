...
def draw(num_count):  # function to move cards from the game deck into your hand
    ...
    if len(game_deck) == 0: #checks for an empty draw deck and displays an error if you draw from it
        ...
        if int(len(game_deck)) < num_count: #checks that you aren't trying to draw more cards than exist in the deck
            ...
            print(p.playername, "drew", num_count, "cards.")
            for i in range(num_count):  # I counts the number of draw_count specified
                if p.playernumber == 1: player1.cardsinhand.append(game_deck.pop(0))
                if p.playernumber == 2: player2.cardsinhand.append(game_deck.pop(0))
...
def count_hand():
    ...
    if p == 1:
        if len(player1.cardsinhand) == 0:
            ...
            print("number of cards in your hand are:", len(player1.cardsinhand))
    if p == 2:
        if len(player2.cardsinhand) == 0:
            print ("There are no cards in your hand.\n")
        else:
            print("number of cards in your hand are:", len(player2.cardsinhand))
...
def show_hand():
    ...
    if p.playernumber == 1:
        if len(player1.cardsinhand) == 0:
            ...
            print("Cards in", p.playername, "'s hand are:")
            for i in player1.cardsinhand:
                print (i.name)
    if p.playernumber == 2:
        if len(in_hand) == 0:
            print("There are no cards in your hand.\n")
